...
def _lower_lazybuffer(outs:List[LazyBuffer], realizes:Dict[LazyBuffer, None]) -> Tuple[ScheduleItem, Dict[Variable, int]]:
  ...
  if (out:=outs[0]).op in {MetaOps.CUSTOM, MetaOps.COPY, MetaOps.EMPTY, MetaOps.VIEW}:
    return ScheduleItem(UOp(UOps.EXT, out.dtype, (), (out.op, out.arg)), (out.buffer,)+tuple(x.base.buffer for x in out.srcs)), {}
  # create the stores
  ...
  return ScheduleItem(sink, tuple(x.buffer for x in outs+inputs),
                      tuple(dedup([x[0].metadata for x in cache if x[0].metadata and x[0] not in inputs]))), var_vals
...
def _graph_schedule(outs:List[LazyBuffer]) -> \
  ...
  output_groups, realizes = _get_output_groups(outs)
...
def create_schedule_with_vars(outs:List[LazyBuffer]) -> Tuple[List[ScheduleItem], Dict[Variable, int]]:
  ...
  while queue:
    lsi = queue.popleft()
    schedule.append(ScheduleItem(lsi.ast, tuple(x for x in lsi.bufs if x.size != 0), lsi.metadata))
    for x in graph[lsi]:
      in_degree[x] -= 1
